...
function [Mid,Did,Dva] = ISLAB_6BJe(B,C,D,F,nk,N,sigma,lambda) 
%
% ISLAB_6B   Module that estimates an BJ model by using MCMMPE. 
%
% Inputs:	F      # true coefficients of AR part 
%                        ([1 1.5 0.7], by default)
%        	B      # true coefficients of X part 
%                        ([1 0.5], by default)
%        	C      # true coefficients of MA part (noise filter)
%                        ([1 -1 0.2], by default)
%           D      # true coefficients of AR part (noise filter)
%                        ([1 -1.5 0.7], by default)
...
Nf = 5 ; 			% Maximum index of AR part.  
Nb = 5 ;			% Maximum index of X part. 
Nc = 5 ;			% Maximum index of MA part. 
Nd = 5 ; 			% Maximum index of AR part.
...
for nb=1:Nb
  ...
  for nc=1:Nc
    ...
    for nd=1:Nd
      ...
      if ((nf>1) || (nb>1) || (nc>1) || (nd > 1)) % Model estimation.
         ...
         if ((nb>1) && (nc>1) && (nd>1) && (nf>1))	% Upgrade optimal indices. 
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
disp(' ') ;
	nb = input('# Insert optimal indices [nb nc nd nf]: ') ; 
nb = abs(round(nb(1:4))) ; 
nf = min(Nf,nb(4))+1 ;
nd = min(Nd,nb(3))+1 ; 
nc = min(Nc,nb(2))+1 ; 
